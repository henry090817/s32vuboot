...
int cse_init(void)
{
	uint32_t firmware;
	uint32_t err;
    uint8_t *pKeyImage = (uint8_t *)(KIA_BASE);
...
cse_firmware_loading:
    ...
    pKeyImage[2] = 0x00;
    pKeyImage[3] = 0x00;
    pKeyImage[4] = (CSE_BLOB_BASE >> 24) & 0x000000ff;
    pKeyImage[5] = (CSE_BLOB_BASE >> 16) & 0x000000ff;
    pKeyImage[6] = (CSE_BLOB_BASE >> 8) & 0x000000ff;
    pKeyImage[7] = (CSE_BLOB_BASE >> 0) & 0x000000ff;
...
//	writel(virt_to_phys(&firmware), CSE_P1);
	...
	/* Open KRAM *///add by henry for cse @20180315
